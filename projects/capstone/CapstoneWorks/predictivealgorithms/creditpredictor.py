...
# train, test, validate = np.split(df_reduced, [int(.6*len(df)), int(.8*len(df))])
# 
...
dfx = df.iloc[:,0:23]
dfy = df['Y']
dfrep = pd.DataFrame(prep.sum(axis = 1),columns =['delay_sum'])

#plt.hist(x = dfrep['delay_sum'])
...
prep = pd.DataFrame(pc,columns=['lpc1','lpc2','lpc3','lpc4'])
...
covar_matrix = PCA(n_components = 6)
covar_matrix.fit(df.iloc[:,17:23])
variance = covar_matrix.explained_variance_ratio_
var=np.cumsum(np.round(covar_matrix.explained_variance_ratio_, decimals=3)*100)
#print var
...
pca = PCA(n_components = 4)
pc = pca.fit_transform(df.iloc[:,17:23])
prep = pd.DataFrame(pc,columns=['rpc1','rpc2','rpc3','rpc4'])
...
df_reduced = pd.concat([df.iloc[:,0:5],dfrep,pdue,prep,df['Y']],axis=1)
#plt.show()

covar_matrix = PCA(n_components = 12)
covar_matrix.fit(df_reduced.iloc[:,0:12])
variance = covar_matrix.explained_variance_ratio_
var=np.cumsum(np.round(covar_matrix.explained_variance_ratio_, decimals=3)*100)
#print var
...
print dftab.sort_values(by='Precision', ascending=False)
